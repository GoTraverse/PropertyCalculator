...
const OG_DIR = path.join(ROOT, 'images', 'suburbs');
...
const CACHE_OG = path.join(CACHE_DIR, 'og-suburbs');
...
function restoreFromCache() {
  ...
  if (fs.existsSync(CACHE_OG)) copyDir(CACHE_OG, OG_DIR);
...
function saveToCache() {
  ...
  if (fs.existsSync(CACHE_OG)) execSync(`rm -rf "${CACHE_OG}"`, { stdio: 'inherit' });
  copyDir(SUBURB_DIR, CACHE_SUBURB);
  copyDir(INVEST_DIR, CACHE_INVEST);
  if (fs.existsSync(OG_DIR)) copyDir(OG_DIR, CACHE_OG);
...
async function buildSuburbs() {
  ...
  // Step 2: Generate suburb-specific OG images (social sharing cards)
  try {
    const ogGen = require('./build/generate-og-images.js');
    await ogGen.main();
  } catch (e) {
    console.warn('[build] OG image generation failed (non-fatal):', e.message);
  }

  // Step 3: Build all suburb HTML pages from updated suburbs.json
