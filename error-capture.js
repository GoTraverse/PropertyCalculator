...
(function () {
  ...
  document.addEventListener('securitypolicyviolation', function (e) {
    ...
    // Drop wasm-eval blocks — always caused by browser extensions, never by our code
    if (blocked === 'wasm-eval') return;
    // Drop Facebook in-app browser injecting its own scripts (pcm.js, fbevents.js)
    if (blocked.indexOf('connect.facebook.net') !== -1) return;
    // Drop Google Ads audience pings from country-specific TLDs we haven't whitelisted
    if (blocked.indexOf('/ads/ga-audiences') !== -1) return;
