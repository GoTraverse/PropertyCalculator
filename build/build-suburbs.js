...
for (const s of suburbs) {
  ...
  let html = SUBURB_TPL
    ...
    .replace(/\{\{OG_IMAGE_URL\}\}/g, `https://equitysight.app/.netlify/functions/og-image?suburb=${encodeURIComponent(s.suburb)}&state=${encodeURIComponent(s.state)}&postcode=${encodeURIComponent(pc)}`)
...
for (const [cityName, cityDef] of Object.entries(CITY_DEFS)) {
  ...
  let cityHtml = CITY_TPL
    ...
    .replace(/\{\{META_DESCRIPTION\}\}/g, escHtml(metaDesc))
    .replace(/\{\{OG_IMAGE_URL\}\}/g, `https://equitysight.app/.netlify/functions/og-image?suburb=${encodeURIComponent(cityName)}&state=${encodeURIComponent(state)}`)
    .replace(/\{\{DATA_SOURCE_NOTE\}\}/g, escHtml(dataSourceNote))
