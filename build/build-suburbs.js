...
for (const s of suburbs) {
  ...
  let html = SUBURB_TPL
    ...
    .replace(/\{\{OG_IMAGE_URL\}\}/g, `https://equitysight.app/.netlify/functions/og-image?suburb=${encodeURIComponent(s.suburb)}&amp;state=${encodeURIComponent(s.state)}${pc ? '&amp;postcode=' + encodeURIComponent(pc) : ''}`)
...
for (const [cityName, cityDef] of Object.entries(CITY_DEFS)) {
  ...
  let cityHtml = CITY_TPL
    ...
    .replace(/\{\{OG_IMAGE_URL\}\}/g, `https://equitysight.app/.netlify/functions/og-image?city=${encodeURIComponent(cityName)}&amp;state=${encodeURIComponent(state)}`)
    .replace(/\{\{META_DESCRIPTION\}\}/g, escHtml(metaDesc))
    .replace(/\{\{DATA_SOURCE_NOTE\}\}/g, escHtml(dataSourceNote))
...
for (const state of allStates) {
  ...
  let html = HUB_TPL
    ...
    .replace(/\{\{OG_IMAGE_URL\}\}/g, `https://equitysight.app/.netlify/functions/og-image?suburb=${encodeURIComponent(stateName)}&amp;state=${encodeURIComponent(state)}`)
