...
for (const s of suburbs) {
  ...
  let html = SUBURB_TPL
    ...
    .replace(/\{\{OG_IMAGE_URL\}\}/g, `https://equitysight.app/.netlify/functions/og-image?type=suburb&state=${encodeURIComponent(s.state.toLowerCase())}&slug=${encodeURIComponent(s.slug)}&postcode=${encodeURIComponent(pc)}`)
...
for (const [cityName, cityDef] of Object.entries(CITY_DEFS)) {
  ...
  let cityHtml = CITY_TPL
    ...
    .replace(/\{\{OG_IMAGE_URL\}\}/g, `https://equitysight.app/.netlify/functions/og-image?type=city&state=${encodeURIComponent(cStateLower)}&slug=${encodeURIComponent(cSlug)}`)
