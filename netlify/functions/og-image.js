'use strict';

/**
 * og-image.js — Dynamic OG image generator for suburb pages.
 * Returns an SVG (image/svg+xml) branded card with suburb name, state, and postcode.
 * URL: /.netlify/functions/og-image?suburb=Paddington&state=QLD&postcode=4064
 */

function escSvg(s) {
  return String(s || '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;').replace(/'/g, '&#39;');
}

function truncate(s, max) {
  if (s.length <= max) return s;
  return s.slice(0, max - 1) + '…';
}

exports.handler = async (event) => {
  const params = event.queryStringParameters || {};
  const suburb = params.suburb || 'Suburb';
  const state = params.state || '';
  const postcode = params.postcode || '';

  const displaySuburb = truncate(suburb, 28);
  const subtitle = [state, postcode].filter(Boolean).join(' ');

  const suburbFontSize = displaySuburb.length > 20 ? 56 : displaySuburb.length > 14 ? 64 : 72;

  const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 630" width="1200" height="630">
  <defs>
    <radialGradient id="glowA" cx="50%" cy="50%" r="50%">
      <stop offset="0%" stop-color="#F2C94C" stop-opacity="0.12"/>
      <stop offset="70%" stop-color="#F2C94C" stop-opacity="0"/>
    </radialGradient>
    <radialGradient id="glowB" cx="50%" cy="50%" r="50%">
      <stop offset="0%" stop-color="#F2C94C" stop-opacity="0.07"/>
      <stop offset="70%" stop-color="#F2C94C" stop-opacity="0"/>
    </radialGradient>
    <pattern id="dots" x="0" y="0" width="32" height="32" patternUnits="userSpaceOnUse">
      <circle cx="1" cy="1" r="1" fill="rgba(255,255,255,0.04)"/>
    </pattern>
  </defs>

  <rect width="1200" height="630" fill="#1A1A1A"/>
  <rect width="1200" height="630" fill="url(#dots)"/>
  <circle cx="1080" cy="100" r="260" fill="url(#glowA)"/>
  <circle cx="120" cy="550" r="200" fill="url(#glowB)"/>

  <!-- Logo top-left -->
  <g transform="translate(72,64)">
    <g transform="scale(0.4)">
      <rect x="40.4" y="124.66" width="17.64" height="31.58" rx="5.64" fill="rgba(255,255,255,0.28)"/>
      <rect x="70.14" y="105.71" width="17.64" height="50.53" rx="5.64" fill="rgba(255,255,255,0.52)"/>
      <rect x="99.88" y="82.02" width="17.64" height="74.22" rx="5.64" fill="#F2C94C"/>
      <rect x="103.32" y="27.93" width="18.48" height="29.48" rx="2.77" fill="#FFFFFF"/>
      <path d="M 26.88 77.28 Q 16.8 77.28 24.54 70.83 L 72.24 31.08 Q 84 24.36 95.76 31.08 L 143.46 70.83 Q 151.2 77.28 141.12 77.28 Z" fill="#FFFFFF"/>
    </g>
    <text x="90" y="58" font-family="'Sora','Helvetica Neue',Arial,sans-serif" font-weight="600" font-size="46" letter-spacing="-1.2" fill="#FFFFFF">Equity</text>
    <text x="226" y="58" font-family="'Sora','Helvetica Neue',Arial,sans-serif" font-weight="600" font-size="46" letter-spacing="-1.2" fill="#F2C94C">Sight</text>
  </g>

  <!-- Suburb name -->
  <text x="72" y="340" font-family="'Sora','Helvetica Neue',Arial,sans-serif" font-weight="700" font-size="${suburbFontSize}" letter-spacing="-2" fill="#FFFFFF">${escSvg(displaySuburb)}</text>

  <!-- State + postcode -->
  <text x="72" y="400" font-family="'DM Sans','Helvetica Neue',Arial,sans-serif" font-weight="500" font-size="36" fill="#F2C94C">${escSvg(subtitle)}</text>

  <!-- Category label -->
  <text x="72" y="460" font-family="'DM Sans','Helvetica Neue',Arial,sans-serif" font-weight="400" font-size="24" fill="rgba(255,255,255,0.5)">Property Investment Profile</text>

  <!-- URL -->
  <text x="72" y="570" font-family="'DM Sans','Helvetica Neue',Arial,sans-serif" font-weight="400" font-size="22" letter-spacing="0.5" fill="rgba(255,255,255,0.5)">equitysight.app</text>

  <!-- Large ghost mark right side -->
  <g transform="translate(900,320)" opacity="1">
    <rect x="48" y="150" width="42" height="76" rx="13" fill="rgba(242,201,76,0.15)"/>
    <rect x="120" y="105" width="42" height="121" rx="13" fill="rgba(242,201,76,0.28)"/>
    <rect x="192" y="60" width="42" height="166" rx="13" fill="rgba(242,201,76,0.55)"/>
    <rect x="200" y="-70" width="44" height="72" rx="7" fill="#F2C94C"/>
    <path d="M 22 120 Q 0 120 18 104 L 132 12 Q 160 -4 188 12 L 302 104 Q 320 120 298 120 Z" fill="rgba(255,255,255,0.07)"/>
  </g>
</svg>`;

  return {
    statusCode: 200,
    headers: {
      'Content-Type': 'image/svg+xml',
      'Cache-Control': 'public, max-age=31536000, immutable'
    },
    body: svg
  };
};
